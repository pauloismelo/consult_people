...
if ($_FILES['arquivo']['name']!=''){ //verifica se houve tentativa de subir a imagem
	...
	// Primeiro verifica se deve trocar o nome do arquivo
        ...
        */
        $nome_final = date('Ymd_his').'.'.$extensao;
		...
		if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
			...
			$query = "update dados set payment_email='".$_POST['payment_email']."', payment_senha='".$_POST['payment_senha']."', payment_token='".$_POST['payment_token']."', layout='".$_POST['layoutx']."', serasa_login='".$_POST['serasa_login']."', serasa_senha='".$_POST['serasa_senha']."', fonte='".$_POST['fonte']."', vlr_consulta='".$_POST['vlr_consulta']."', contrato='".$nome_final."' where id=".$_POST['id']." ";
            if (mysqli_query($conn, $query)) {
                echo "<script>alert('Alterações realizadas com sucesso!');</script>";
                echo "<script>window.location='painel.php?go=config';</script>";
            } else {
    ...
    $query = "update dados set payment_email='".$_POST['payment_email']."', payment_senha='".$_POST['payment_senha']."', payment_token='".$_POST['payment_token']."', layout='".$_POST['layoutx']."', serasa_login='".$_POST['serasa_login']."', serasa_senha='".$_POST['serasa_senha']."', fonte='".$_POST['fonte']."', vlr_consulta='".$_POST['vlr_consulta']."' where id=".$_POST['id']." ";
    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Alterações realizadas com sucesso!');</script>";
        echo "<script>window.location='painel.php?go=config';</script>";
    } else {
